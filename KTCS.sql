...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ktcs`.`view_cars_by_location`  AS  select `ktcs`.`car`.`make_model` AS `make_model`,`ktcs`.`car`.`year` AS `year`,`ktcs`.`car`.`rental_fee` AS `rental_fee`,`ktcs`.`car`.`dropoff_location` AS `pick_up_from` from `ktcs`.`car` order by `ktcs`.`car`.`dropoff_location` ;
